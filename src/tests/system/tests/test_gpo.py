...
def test_gpo__is_set_to_enforcing(client: Client, samba: Samba, method: str):
    """
    ...
    """
    samba.user("user").add()
    user1 = samba.user("user1").add()
    user2 = samba.user("user2").add()
    deny_user1 = samba.user("deny_user1").add()
    deny_user2 = samba.user("deny_user2").add()
    group = samba.group("group").add().add_members([user2])
    deny_group = samba.group("deny_group").add().add_members([deny_user2])

    gpo = samba.gpo("site policy").add()

    gpo.policy(
        {
            "SeInteractiveLogonRight": [user1, group, samba.group("Domain Admins")],
